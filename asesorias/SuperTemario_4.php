...
session_start();
$_SESSION['id'];
$sql = 'SELECT * FROM usuarios INNER JOIN temario WHERE( usuarios.id_usuario = temario.id_asesor)and (usuarios.id_usuario = '.$_SESSION['id'].' ) and (temario.estatus = "espera")';
  ...
  $a=1;
   ...
   <div class="col-md-10 " style=" word-wrap: break-word;">
     ...
     <h6> Subido por :   '.$nombre['nombre'].' '.$nombre['apellido_paterno'].' '.$nombre['apellido_materno'].'</h6> </div>
        ...
        echo ' <button id="todos" name="todos" type="submit" value="todos" onclick = "location=\'borrar.php?variable1='.$row['direccion_doc'].'\'"class="btn btn-primary btn-sm" ;>Borrar</button>
        <button id="todos" name="todos" type="submit" value="todos" onclick="editar('.$row['id_documento'].')" class="btn btn-primary btn-sm" ;>Editar descripcion</button>
  </div>';
