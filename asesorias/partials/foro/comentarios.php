...
<ul class="collapsible" data-collapsible="accordion" >
            ...
            <li ng-repeat="data in filteredComentario">
                ...
                <div class="col s1">
                <img class="circle responsive-img" style="width: 40px; height: 40px;" src="{{data.foto}}"> 
                </div>
                <span class="blue-text text-darken-2">{{data.usuario}}:</span>
                <span>{{data.comentario}}</span>
                <span am-time-ago="data.fecha_comentario" class="grey-text"></span>
                <a href="#/insertar-subcomentario/{{data.id_foro}}" class=""><i class="glyphicon glyphicon-plus"></i></a>
                <span class="blue-text" ng-show="((subComentarios|filter:{id_foro:data.id_foro}).length) >'0'">{{(subComentarios|filter:{id_foro:data.id_foro}).length}}</span>
                </div>
                ...
                <div class="collapsible-body"><p class="flow-text" style="">{{data.comentario}}</p>
                <input type="hidden" ng-model="comprueba" />
                <div ng-show="comprueba" style="">
                <a href="#/editar-comentario/{{data.id_foro}}" class=""><i class="glyphicon glyphicon-edit"></i></a>
                <span class="grey-text" am-time-ago="data.fecha_comentario"></span>
                </div>
                    <ul class=" collection " >
                        <li ng-repeat="datas in subComentarios | filter :{id_foro : data.id_foro}" class="col s11 offset-s1 collection-item avatar grey lighten-5">
                            ...
                            <input name="name" value="{{comprueba2=(session2==datas.id_usuario) || (tipo_usuario=='Administrador')}}" type= "hidden" class= "form-control" required/>
