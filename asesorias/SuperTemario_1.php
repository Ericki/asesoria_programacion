...
if ($nombre1['tipo_usuario']=="Asesor") {
  $sql1 = 'SELECT * FROM usuarios INNER JOIN temario WHERE usuarios.id_usuario = temario.id_asesor ';
}else{
  $sql1 = 'SELECT * FROM `temario`';
}
...
if ($a==1) {
   ...
   <button id="todos" name="todos" type="submit" value="todos" onclick="descriocion('.$row['id_documento'].')" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModal" ;>descripcion 1</button>
      <br/>
   </div> 
